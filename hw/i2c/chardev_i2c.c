...
static int chardev_i2c_event(I2CSlave *s, enum i2c_event event)
{
    ...
    case I2C_FINISH:
        ...
        {
            const uint8_t dst_addr = state->tx_buf[0];
            /* destination address will be sent in the header */
            ...
            if (rc == sizeof(hdr))
            {
                ...
                rc = qemu_chr_fe_write_all(&state->chardev, &state->tx_buf[1], state->tx_buf_len);
                trace_chardev_i2c_chardev_write(DEVICE(s)->canonical_path,
                                                 "data", state->tx_buf_len, rc);
                if (rc == state->tx_buf_len)
                {
                    ret = 0;
                } else
                {
