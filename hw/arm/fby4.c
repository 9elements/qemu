...
#define TYPE_FBY4 MACHINE_TYPE_NAME("fby4")
OBJECT_DECLARE_SIMPLE_TYPE(Fby4State, FBY4);
...
#define FBY4_BMC_RAM_SIZE (2 * GiB)
#define FBY4_BMC_FIRMWARE_ADDR 0x0

static void fby4_bmc_write_boot_rom(DriveInfo *dinfo, MemoryRegion *mr,
...
static void fby4_bmc_init(Fby4State *s) {
  AspeedSoCState *soc;
  AspeedSoCClass *sc;
  ...
  memory_region_init(&s->bmc_memory, OBJECT(&s->bmc), "bmc-memory", UINT64_MAX);
  memory_region_add_subregion(get_system_memory(), 0, &s->bmc_memory);
  memory_region_init_ram(&s->bmc_dram, OBJECT(&s->bmc), "bmc-dram",
                         FBY4_BMC_RAM_SIZE, &error_abort);

  object_property_set_int(OBJECT(&s->bmc), "ram-size", FBY4_BMC_RAM_SIZE,
                          &error_abort);
  ...
  if (!s->mmio_exec) {
    ...
    if (mtd0) {
      ...
      fby4_bmc_write_boot_rom(mtd0, &s->bmc_boot_rom, FBY4_BMC_FIRMWARE_ADDR,
                              rom_size, &error_abort);
    }
  }
}

static void fby4_bic_init(Fby4State *s) {
  AspeedSoCState *soc;
  AspeedSoCClass *sc;
  ...
  memory_region_init(&s->bic_memory, OBJECT(&s->bic), "bic-memory", UINT64_MAX);
...
static void fby4_init(MachineState *machine) {
  Fby4State *s = FBY4(machine);

  fby4_bmc_init(s);
  fby4_bic_init(s);
}

static bool fby4_get_mmio_exec(Object *obj, Error **errp) {
  return FBY4(obj)->mmio_exec;
}

static void fby4444_set_mmio_exec(Object *obj, bool value, Error **errp) {
  FBY4(obj)->mmio_exec = value;
}

static void fby4_instance_init(Object *obj) { FBY4(obj)->mmio_exec = false; }

static void fby4_class_init(ObjectClass *oc, const void *data) {
  MachineClass *mc = MACHINE_CLASS(oc);

  mc->desc = "Meta Platforms fby4";
  ...
  object_class_property_add_bool(oc, "execute-in-place", fby4_get_mmio_exec,
                                 fby4_set_mmio_exec);
  object_class_property_set_description(oc, "execute-in-place",
                                        "boot directly from CE0 flash device");
}

static const TypeInfo fby4_types[] = {
    {
        .name = MACHINE_TYPE_NAME("fby4"),
        .parent = TYPE_MACHINE,
        .class_init = fby4_class_init,
        .instance_size = sizeof(Fby4State),
        .instance_init = fby4_instance_init,
        .interfaces = arm_machine_interfaces,
    },
};

DEFINE_TYPES(fby4_types);
